...
miniflow_extract(struct dp_packet *packet, struct miniflow *dst)
{
    ...
    if (OVS_LIKELY(!(nw_frag & FLOW_NW_FRAG_LATER))) {
        ...
        /* voyager: extract from packet into miniflow */
        else if (OVS_LIKELY(nw_proto == 192)) {
            if (OVS_LIKELY(size >= sizeof(struct in6_addr))) {
                const struct in6_addr *voyager_test_hdr;
                voyager_test_hdr = data_pull(&data, &size, sizeof *voyager_test_hdr);
                miniflow_push_words(mf, voyager, voyager_test_hdr,
                                    sizeof *voyager_test_hdr / sizeof(uint64_t));
            }
        }
        /* end - voyager */
    }
