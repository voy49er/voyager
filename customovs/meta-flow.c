...
mf_is_all_wild(const struct mf_field *mf, const struct flow_wildcards *wc)
{
    ...
    /* voyager: mf_is_all_wild() */
    case MFF_VOYAGER:
        return ipv6_mask_is_any(&wc->masks.voyager);
    /* end - voyager */
             ...
             union mf_value *value)
{
    ...
    /* voyager: mf_get_value() */
    case MFF_VOYAGER:
        value->ipv6 = flow->voyager;
        break;
    /* end - voyager */
             ...
             const union mf_value *value, struct match *match, char **err_str)
{
    ...
    /* voyager: mf_set_value() */
    case MFF_VOYAGER:
        match->flow.voyager = value->ipv6;
        match->wc.masks.voyager = in6addr_exact;
        break;
    /* end - voyager */
                  ...
                  const union mf_value *value, struct flow *flow)
{
    ...
    /* voyager: mf_set_flow_value() */
    case MFF_VOYAGER:
        flow->voyager = value->ipv6;
        break;
    /* end - voyager */
...
mf_set_wild(const struct mf_field *mf, struct match *match, char **err_str)
{
    ...
    /* voyager: mf_set_wild() */
    case MFF_VOYAGER:
        memset(&match->wc.masks.voyager, 0, sizeof match->wc.masks.voyager);
        memset(&match->flow.voyager, 0, sizeof match->flow.voyager);
        break;
    /* end - voyager */
       ...
       struct match *match, char **err_str)
{
    ...
    /* voyager: mf_set() */
    case MFF_VOYAGER:
        match->flow.voyager = ipv6_addr_bitand(&value->ipv6, &mask->ipv6);
        match->wc.masks.voyager = mask->ipv6;
        break;
    /* end - voyager */
